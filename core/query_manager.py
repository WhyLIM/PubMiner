    ...
    def create_example_config(self, output_file: str = "query_config_example.json") -> None:
        ...
        example_config = {
            ...
            "query_tasks": [{
                ...
                "query":
                "(COVID-19[ti] OR SARS-CoV-2[ti]) AND (diabetes[ti] OR diabetic[ti]) AND (\"2020/01/01\"[Date - Publication] : \"2024/12/31\"[Date - Publication])",
                "max_results": 50,
                ...
                "name":
                "Aging Biomarkers Study (All Results)",
                "query":
                "(aging[ti] OR senescence[ti]) AND (biomarker[ti] OR marker[ti]) AND (\"2023/01/01\"[Date - Publication] : \"2024/12/31\"[Date - Publication])",
                "max_results":
                None,
                "include_fulltext":
                True,
                "output_file":
                "aging_biomarkers_all.csv",
                "description":
                "Aging-related biomarker research (获取所有结果)",
                "custom_fields": [
                    "What biomarkers are studied in this article", "Single biomarker or combination of biomarkers",
                    "Biomarker category (Protein/DNA/RNA/Other)", "Study population ethnicity", "Gender ratio of study samples"
                ]
            }, {
                "name": "Cancer Immunotherapy (Unlimited)",
                "query":
                "cancer[ti] AND immunotherapy[ti] AND (\"2024/01/01\"[Date - Publication] : \"2024/12/31\"[Date - Publication])",
                "max_results": -1,
                "include_fulltext": False,
            ...
            try:
                ...
                if custom_fields:
                    ...
                    for j, field in enumerate(custom_fields, 1):
                        ...
                        custom_template["fields"][field_key] = {"description": field, "type": "string", "required": False}
                    ...
                    results = pubminer.analyze_by_query(query=query,
