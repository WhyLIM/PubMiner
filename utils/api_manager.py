...
class APIManager:
    ...
    def __init__(self):
        ...
        self.session.headers.update({
            'User-Agent': 'PubMiner/1.0 (Literature Analysis Tool)'
        })
                    ...
                    try:
                        ...
                        if e.response.status_code in retry_on_status:
                            if attempt < max_retries:
                                ...
                                logger.warning(f"API调用失败 (状态码: {e.response.status_code})，"
...
def setup_api_rate_limits():
    """设置API限流规则"""
    # PubMed API限制（无API key时每秒3次，有API key时每秒10次）
    api_manager.add_rate_limiter('pubmed_no_key', 3, 1)
    api_manager.add_rate_limiter('pubmed_with_key', 10, 1)
    
    # LLM API限制（根据不同提供商设置）
    api_manager.add_rate_limiter('openai', 60, 60)  # 每分钟60次
    api_manager.add_rate_limiter('deepseek', 200, 60)  # 每分钟200次
    api_manager.add_rate_limiter('qwen', 100, 60)  # 每分钟100次
    api_manager.add_rate_limiter('volcengine', 60, 60)  # 每分钟60次
