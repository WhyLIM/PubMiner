...
class RichLogger:
    ...
    def __init__(self,
        ...
        # 创建 Rich 控制台
        self.console = Console(theme=PUBMINER_THEME, width=console_width, force_terminal=True, color_system="auto")
    ...
    def print_header(self, title: str, subtitle: str = None):
        ...
        panel = Panel(Align.center(header_text), box=box.DOUBLE, style="cyan", padding=(1, 2))
    ...
    def print_status_panel(self, status_data: dict):
        ...
        for key, value in status_data.items():
            if isinstance(value, dict):
                ...
                panel = Panel(nested_text, title=f"[bold]{key}[/bold]", border_style="blue", padding=(0, 1))
            else:
                # 简单状态
                panel = Panel(f"[number]{value}[/number]", title=f"[bold]{key}[/bold]", border_style="green", padding=(0, 1))
    ...
    def print_error_details(self, error: Exception, context: str = None):
        """打印错误详情"""
        error_panel = Panel(f"[error]{type(error).__name__}: {str(error)}[/error]",
                            title="[bold red] 错误详情 [/bold red]",
                            border_style="red",
                            padding=(1, 2))

        if context:
            context_panel = Panel(f"[dim]{context}[/dim]",
...
def setup_rich_logger(level: int = logging.INFO,
                      log_dir: Optional[Path] = None,
                      console_width: Optional[int] = None,
                      show_time: bool = True,
                      show_path: bool = False) -> RichLogger:
    """
    设置全局 Rich 日志器
    ...
    _rich_logger_instance = RichLogger(level=level,
                                       log_dir=log_dir,
...
def print_welcome():
    ...
    logger.print_header("🔬 PubMiner - 智能文献分析工具", "基于大语言模型的模块化文献挖掘系统")
