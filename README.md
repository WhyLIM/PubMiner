...
PubMiner 采用全新的模块化配置架构，将配置按功能分离，提供更好的组织性和可维护性。
...
#### 查询模板配置 (`config/query/query_templates.json`)

```json
{
    "query_tasks": [
        {
            "id": "covid_diabetes_research",
...
#### 提取模板配置 (`config/extraction/extraction_templates.json`)
...
#### LLM 配置 (`config/core/llm_config.json`)
